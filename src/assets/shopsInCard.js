//Показ наличия (по умолчанию много )
function getAvalible(str){
    if (typeof str === 'string') {
        let Avalcount = str == 'few' ? 'Мало' : 'Много';
        res = { prefix: '__'+str, value: Avalcount};
    } else {
        res = { prefix: '__many', value: 'Много' };
    }
    return res;
}
        ...
        function renderAsidePanel(shopsArray) {
            ...
            shopsArray.forEach(function (shopPoint) {
                ...
                if (typeof shopPoint.url === 'string') {
                    $shopListLink = '<br><a class="more" href="' + shopPoint.url + '">Подробнее</a>';
                }
                ...
                if (typeof shopPoint.address === 'string') {
                    ...
                    let $pin = '<span class="pin"><svg style="fill: ' + pinCol + '" width="14" height="19"><use href="sprite/sprite.svg#pin"></use></svg></span>';
                    $shopListAddress.html($pin + shopPoint.address + $shopListLink);
                ...
                $shopListAvail.addClass('table-cell __avalib flex');
                let aval = getAvalible(shopPoint.available);
                $shopListAvail.html('<div class="aval-value ' + aval.prefix + '">' + aval.value + '</div>');

                let $btn = $('<span class="btn btn-transp r10" href="#">Выбрать</span>').appendTo($shopListAvail);
            ...
            function setShopPoints() {
                ...
                var setshopPointOptions = function () {
                    return {
                        ...
                        iconLayout: 'default#image',
                        // Своё изображение иконки метки.
                        iconImageHref: './img/map/shop-pin.svg',
                ...
                function setClickCommonHandler(placemark) {
                    ...
                    placeMarks.forEach(function (point) {
                        ...
                        point.options.set('iconImageHref', './img/map/shop-pin.svg');
                    ...
                    if (placemark.isVeterinaryClinic){
                        ...
                        placemark.options.set('iconImageHref', './img/map/shop-pin.svg');
                    }
                    ...
                    let aval = getAvalible(object.available);
                    balloon_content += ('<p class="aval-value ' + aval.prefix + '">Наличие в магазине: ' + aval.value + '</p>');
